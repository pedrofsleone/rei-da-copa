...
4. **Fase D — Torneio ASSISTIR (PROXIMA):** host monta grupos/bracket (com regra `spread` de espalhar humanos) e resolve placares (autoritativo); tabela e chaveamento ao vivo; playback cosmetico dos jogos do jogador; campeao. Hoje `tourneyWait` mostra "Fase D em breve" quando todos os times estao prontos.
