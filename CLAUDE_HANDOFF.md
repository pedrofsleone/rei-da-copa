...
Ultima atualizacao: 2026-07-05. NO AR em https://pedrofsleone.github.io/rei-da-copa/ (deploy automatico via git push; ver secao DEPLOY). Ja entregue e deployado antes deste ajuste: Home (Contra CPU / Multiplayer), lobby de torneio configuravel, DRAFT POR TURNOS ao vivo com cartas Bloqueio/Troca e layout de ABAS estilo 7a0 (item 3j), e FASE D.1 = transmissao progressiva sincronizada do torneio (host controla, sem spoiler, campeao so no fim; item 4b). Ajuste local mais recente: tela **Sala de jogo** redesenhada no estilo poster da referencia do Pedro, com topo "Rei da Copa / Sala de jogo / Sair da sala / Opcoes", paineis brancos com borda preta para Sala, Campeonato, Config e Jogadores, arte lateral `assets/room-players.png`, e botoes reais preservados (copiar codigo, config, pronto, comecar). Correcao de publicacao: adicionado `.github/workflows/pages.yml` para o GitHub Pages publicar a raiz da branch `main` por Actions, porque o site publicado mostrou a home antiga escura mesmo com o commit novo no `main`. Base: 16 selecoes com regua propria (RATING_GUIDE.md), 8 formacoes iguais as do 7a0, penaltis na hora. FALTA validar com 2 humanos reais (2 celulares) o draft+transmissao ao vivo; depois: modo Interativo, regras definitivas do Firebase (~2026-08-02), expandir base (COVERAGE_TARGETS.md). Detalhes: roadmap no fim do arquivo (itens numerados ate 3j/4b).
...
- **Ajuste Fase B - Poster de multiplayer (2026-07-05)**: Pedro pediu para trocar a tela escura "Jogar com amigos" pela imagem ilustrada de criar/entrar sala. Implementacao local em `index.html`: `multiHomeView` agora usa `.multi-poster-entry` com fundo `assets/multi-poster.jpg`; `body.is-multi-entry-screen` esconde a topbar nessa tela; `nicknameInput`, `roomCodeInput`, `createRoomBtn`, `joinRoomBtn` e `multiBackBtn` mantem os IDs originais, mas ficam posicionados sobre a arte como campos/botoes funcionais. O primeiro campo continua alimentando `state.nickname`, mas o texto visual passou para "Nome da sala"/"Digite o nome da sala..." para bater com a referencia. Mensagem de erro atual: "Digite o nome da sala primeiro." Falta validar visualmente no telefone real.
- **Ajuste Fase B.2 - Sala de jogo poster (2026-07-05)**: Pedro mandou referencia visual para a tela apos criar/entrar em sala. Implementacao local em `index.html`: `multiLobbyView` foi reconstruida como `.room-poster` mobile-first, com `body.is-room-poster-screen` escondendo a topbar, topo de abas, coluna de paineis `Sala / Campeonato / Config / Jogadores`, botao verde **ESTOU PRONTO**, botao **COMEÇAR** preservado para host e arte lateral recortada em `assets/room-players.png`. `lobbyConfigPanel` agora renderiza paineis `.room-panel` por HTML real, nao imagem estatica; IDs funcionais preservados (`lobbyLeaveBtn`, `copyCodeBtn`, `readyBtn`, `startBtn`, `lobbyConfigPanel`, etc.). Salas novas gravam `room.name` a partir do texto digitado. Validacao local feita: sintaxe JS e IDs OK, `git diff --check` OK, detector Impeccable local `[]`. Falta validar no Chrome/telefone real porque o browser controlado nao estava disponivel neste ambiente.

DEPLOY (Fase H CONCLUIDA): repo GitHub **https://github.com/pedrofsleone/rei-da-copa** (usuario `pedrofsleone`), publicado via **GitHub Pages** em **https://pedrofsleone.github.io/rei-da-copa/** (no ar, mobile-friendly). IMPORTANTE: tem um arquivo **`.nojekyll`** na raiz — sem ele o build do Pages FALHA (Jekyll se engasga com o HTML/JS); nao remover. Deploy de novas versoes: so `git add/commit/push` na pasta `C:\Users\pedro\Desktop\Rei-da-Copa` que o Pages reconstroi sozinho em ~1min. Em 2026-07-05 foi adicionado `.github/workflows/pages.yml` para publicar a raiz da branch `main` via GitHub Actions; se o site continuar mostrando a home antiga escura depois do push, conferir em **Settings > Pages** se a fonte esta em **GitHub Actions** e aguardar a action "Deploy Rei da Copa" ficar verde. Git config: repo local com `safe.directory` marcada (a pasta pertence a outro SID no Windows), identidade Pedro Leone/gmail, remote `origin` = o repo acima, credencial via Git Credential Manager (helper `manager`, ja logado). `gh` CLI NAO instalado.
...
3k. PACOTE 2026-07-06 (pedidos do Pedro): (a) **FASE DE AJUSTE (1 min)** — quando todas as escolhas acabam, em vez de fechar direto o host grava `draft/phase="adjust"` + `deadline=+60s` (`startAdjustPhase`/branch no `hostTimeoutPick`); cada humano pode TROCAR POSICOES tocando em 2 jogadores (`renderTdAdjustField`/`tdAdjustMove`, compatibilidade via `positions` salvo no pick ou lookup por prefixo do id no acervo) e ESCOLHER O CAPITAO (+5, botao no painel `renderTdAdjustPanel`, grava `teams/{me}/captainId`); botao "Estou pronto" grava `adjustDone`; host fecha (`hostAdjustTick`) quando todos confirmam ou o tempo acaba (guarda `state.finishingDraft`). (b) **BONUS NOVOS**: mesma selecao **+5**, mesmo ano **+3**, capitao **+5** (funcao generica `bonusFor` + `playerMeta` que deriva pais/ano do prefixo do id; solo `playerBonus` reusa). `onlineTeamSummary` agora respeita `captainId` escolhido e embute os bonus no overall/ataque/defesa (torneio joga com nota efetiva). (c) **SELOS NOS CHIPS**: `tdSlotBadges` mostra `C` dourado (capitao) e `+N` verde (sincronia) em todos os campos do draft (`renderTdField`, `fillWatchField`, ajuste). CSS `.slot-cap/.slot-bonus`. (d) **TIMEOUT ALEATORIO DA SELECAO SORTEADA**: o sorteio agora vai pro RTDB (`draft/draw`, escrito em `tdRoll/tdReroll` via `tdShareDraw`, limpo a cada pick/advance); `hostTimeoutPick` pega jogador ALEATORIO dessa selecao que caiba (fallback: sorteia selecoes ate achar), em vez do mais forte do acervo. (e) **ROUBO VISIVEL PRA TODOS**: `surfaceTrocaResult` mostra toast pra sala inteira com apelidos (sucesso e bloqueio). (f) **TRANSMISSAO SO DE JOGOS DE PLAYERS**: `hostBroadcastDrive` resolve CPUxCPU em BLOCO instantaneo (nunca aparecem na Central); listas de jogos (geral/grupos/chave) filtradas pra partidas com humano (`bcMatchHasHuman`), tabelas continuam completas; secao "Seus jogos" virou "Jogos dos players" (todas as partidas com humanos). (g) **TELA DO TORNEIO NA IDENTIDADE POSTER CLARA**: `#tourneyWaitView` com o mesmo override de variaveis + fundo `draft-bg.jpg` fixo do draft; `.bc-central` continua card escuro (campo) com texto forcado claro; `is-mine/is-qualified` com verde claro. Validado: JS_OK e teste VM (capitao respeitado, bonus +8/+8, meta por id, overall com bonus 98 vs 92 cru). FALTA validar ao vivo com 2 celulares.
3l. CORRECOES 2026-07-06 na transmissao (teste real do Pedro): (a) BUG GRAVE corrigido — `hostBroadcastDrive` roda no `routeTournament` a CADA update do banco, entao com Auto ligado cada patch disparava outro avanco em cascata e o campeonato inteiro revelava na hora. Agora ha um gate de tempo (`bcLastEvAt`, 1 lance a cada ~1.5s) e no FIM do jogo o Auto desliga sozinho (`auto:false`) em vez de pular pro proximo. (b) Pular continua revelando so a PARTIDA atual. (c) TRAVA "TODOS VIRAM": ao fim de cada jogo humano, cada player VIVO confirma "✓ Visto" (`bcVisto` grava `broadcast/seen/{clientId}=index`); o host so avanca quando todos os necessarios confirmaram (`bcHumansNeedingSeen`: humanos conectados com jogo atual/futuro na ordem — quem ja caiu do campeonato ou desconectou NAO trava). UI: botao Visto + contador "N/M confirmaram" pra todos; controles Auto/Pular/Lance so durante o jogo. (d) LANCES legiveis: no `#tourneyWaitView` a `.bc-timeline` perdeu a caixa preta (fundo translucido claro) e os `.event` viraram pilulas brancas com texto escuro e minuto verde. Validado: JS_OK + teste VM do gate (so vivos confirmam; desconectado nao trava). FALTA validar ao vivo com 2 celulares.
3m. FEATURES 2026-07-06 na transmissao (respostas do Pedro via AskUserQuestion: livre total / painel completo / entre placar e lances / ~1 min por jogo): (a) TIMELINE: lance mais novo PRIMEIRO com destaque `.event.is-latest` (a caixa rolavel escondia os novos). (b) CONFIG `playback` no lobby ("Transmissao": `host` = anfitriao controla sincronizado | `livre` = "Cada um no seu ritmo"); default host; em `cfgOr`, `configEditorHtml/configReadonlyHtml` e no default do `createRoom`. (c) MODO LIVRE: cada aparelho dirige o proprio playback local (`state.localBc`, driver `driveLocalBc` com as mesmas regras do host; `bcResolved(room)` decide local vs `room.broadcast`; controles Auto/Pular/Lance pra TODOS + "Proxima partida" no fim; SEM trava "todos viram"; `hostBroadcastDrive` early-return e `broadcastTick` chama o driver local). Controles `bcLance/bcAuto/bcPular/bcProximo` viraram mode-aware. (d) RITMO: `bcAutoGap(events)` = 60s/lances (clamp 6-20s) => partida ~1 min no Auto (host e livre). (e) PAINEL DE PRESSAO (`renderPressure`, entre placar e lances no card verde): campo com onda de momentum (`.bc-wave` avanca pro lado que pressiona), linha central, PASSES pontilhados em SVG com bola destacada, e "Posse de bola + nome do jogador" do elenco real; cronometro cosmetico no titulo ("AO VIVO · 23'") via `bcSimMinute` (interpola minuto entre lances no ritmo do gap). TUDO CENOGRAFICO e DETERMINISTICO: PRNG `seededRand(hashStr(match.id)*97 + minuto*13)` => todos os aparelhos veem a MESMA pressao; vies de 8 min pro time do proximo gol. Validado em VM: gaps 12s/20s/6s, html deterministico, vies OK. FALTA validar ao vivo (2 celulares) e o modo livre de ponta a ponta.
3n. FEATURES 2026-07-07 (pedidos do Pedro): (a) **DIFICULDADE por sala** (config `difficulty` no lobby, default `peladeiro`): Perna de pau (~70% de vitorias vs CPU) / Peladeiro (~50%) / Profissional (~20%) / Craque (~7,5%). Implementacao: `DIFFICULTY_DELTA = {perna:4, peladeiro:1.5, profissional:-3, craque:-6.2}` — delta de forca somado ao lado HUMANO (overall/ataque/defesa) em jogos humano x CPU dentro de `simulateTournamentMatch` (via `simHumanDelta`, setado em `buildTournament`); humano x humano NAO leva delta. Deltas CALIBRADOS por Monte Carlo na formula real (validado: 70,2/50,8/20,5/6,9% com times iguais de 80; as taxas reais variam com a forca do time draftado). Hints no lobby (`DIFFICULTY_HINT`). (b) **MODO ALMANAQUE** (config `notas`: `classico`/`almanaque`, independente da dificuldade e combinavel com qualquer uma): durante `status:"drafting"`, `tdAlmanaque()` esconde as notas em TODO o draft — lista de escolha (rating vira "?"), chips do campo (`draftPlayerLabel` so a sigla), lista/roubo dos adversarios, e o painel TEAM STATS vira "?" (a media entregaria as notas). Quando o torneio comeca (`running`), as notas aparecem normalmente. CRITERIOS DE VITORIA do simulador (pra referencia): gols de cada lado = `round((ataque_seu - defesa_dele + 18)/11 + aleatorio*2.2)` clamp 0-5 (ataque = media PE/CA/PD/MEI com bonus; defesa = GOL/LD/ZAG/LE/VOL com bonus; capitao +5 e sincronia +5/+3 ja embutidos no resumo do time); empate 0x0 em grupo tem 45% de virar 1x0 pro lado com mais pressao (overall); empate em mata-mata decide por `overall + sorte(-10..10)` e a serie de penaltis mostrada e cosmetica. FALTA validar ao vivo.
